...
def trade():
    ...
    in_position = any(position.symbol == symbol for position in positions)

    # Check available balance for BTC
    btc_position = next((position for position in positions if position.symbol == symbol), None)
    available_btc = 0 if btc_position is None else float(btc_position.qty)
    
    if long_condition and not in_position:
        if position_size <= available_btc:
            place_order('buy', position_size)
            manage_position()
        else:
            print("Insufficient BTC balance for the trade.")
